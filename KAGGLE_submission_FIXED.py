...
FIX: Removed 'brand' categorical encoding (48.5% unseen brands in test)
     Kept 'brand_exists' binary feature + brand interaction features

Performance: 27.86% validation SMAPE
Expected LB: 30-35% SMAPE (after fix)

Features:
- 21 engineered tabular features (removed 'brand', kept 'brand_exists')
...
# Define tabular features - REMOVED 'brand' categorical encoding
# Reason: 48.5% of test samples have unseen brands, causing encoding issues
tabular_features = [
    'value', 'unit', 'pack_size',
    'has_premium', 'has_organic', 'has_gourmet',
    'has_natural', 'has_artisan', 'has_luxury',
    'is_travel_size', 'is_bulk',
    'brand_exists',  # Binary feature - works fine
    'value_x_premium', 'value_x_luxury', 'value_x_organic',
    'pack_x_premium', 'pack_x_value',
    'brand_x_premium', 'brand_x_organic',  # Interaction features still useful
    'travel_x_value', 'bulk_x_value'
]

print(f"✓ Using {len(tabular_features)} tabular features (removed 'brand' encoding)")

# Encode categorical features (only 'unit' is categorical now)
label_encoders = {}
...
print(f"\n{'='*80}")
print("FIX APPLIED: Removed 'brand' categorical encoding")
